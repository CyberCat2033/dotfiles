...
int main() {
    ...
    cv::circle(mask, cv::Point(mask_size / 2, mask_size / 2), mask_size / 2, cv::Scalar(255), -1);
    ...
    while (true) {
        ...
        if (command == "process") {
            ...
            if (image.cols > image.rows) {
                new_width = mask_size;
                new_height = static_cast<int>(image.rows * (static_cast<float>(mask_size) / image.cols));
            } else {
                new_height = mask_size;
                new_width = static_cast<int>(image.cols * (static_cast<float>(mask_size) / image.rows));
            }
            ...
            resized_image.copyTo(square_image(cv::Rect(x_offset, y_offset, new_width, new_height)));
